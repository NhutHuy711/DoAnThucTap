...
function handleFilterChange(filterType, value, checked = null) {
    switch (filterType) {
        case 'brand':
            document.querySelectorAll('.brand-filter').forEach(cb => cb.disabled = true);
            const url = new URL(contextPath + `brand/${encodeURIComponent(value)}`, window.location.origin);
...
function updateFilterTags() {
    ...
    if (currentFilters.priceRange) {
        ...
        switch (currentFilters.priceRange) {
            case 'UNDER_50': priceText = `Under ${formatCurrency('50')}`; break;
            case '50_TO_100': priceText = `${formatCurrency('50')} - ${formatCurrency('100')}`; break;
            case '100_TO_200': priceText = `${formatCurrency('100')} - ${formatCurrency('200')}`; break;
            case 'OVER_200': priceText = `Over ${formatCurrency('200')}`; break;
        }
...
function createFilterTag(text, onRemove) {
    ...
    tag.innerHTML = `${text}<span class="close-btn" onclick="event.stopPropagation();">×</span>`;
